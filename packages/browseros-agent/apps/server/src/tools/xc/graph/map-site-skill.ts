 ...
 *   JSON + MMD:  regenerated every SAVE_INTERVAL pages (default 10) and always
 *               on crawl completion, to avoid excessive I/O on large crawls.
...
/** Maximum chars for an api_call node label (Issue 3 fix). */
const API_CALL_LABEL_MAX = 120

/** Maximum localStorage keys stored as nodes (Issue 10 fix — align with Phase 1 cap of 30). */
...
interface BfsState {
  ...
  rootUrl: string
  visited: Set<string>
  /** Issue 6 fix: O(1) membership check replaces queue.includes() */
  queued: Set<string>
  queue: string[]
  maxDepth: number
  maxPages: number
...
/**
 * Issue 7 fix: hasPricing and hasDocs are now derived from the page URL
 * at the call site, so they are never hardcoded as false.
 */
...
/**
 * Issue 3 fix: Truncate api_call labels to API_CALL_LABEL_MAX chars.
 * Full endpoint is still available in meta.endpoint.
 */
...
/**
 * Issue 4 fix: Extract only the URL path+query for apiCallsObserved,
 * dropping the scheme+host to avoid storing 500-char URLs in page meta.
 */
...
export const map_site_start = defineTool({
  ...
  description: [
    ...
    'NDJSON appended per mutation; JSON+MMD regenerated every 10 pages and on completion.',
    'REQUIRED: url.',
    'OPTIONAL: maxDepth (1-5, default 2), maxPages (1-100, default 50 — use 50-100 for',
    'real sites; 20 only covers top-level nav and misses most product/docs content),',
  ...
  input: z.object({
    ...
    maxPages: z.coerce.number().int().min(1).max(100).default(50)
      .describe('Maximum pages to visit (default: 50, max: 100). Use 50-100 for real sites; 20 is usually insufficient.'),
  ...
  async handler(args, ctx, response) {
    ...
    while (bfsState.queue.length > 0 && bfsState.pagesVisited < args.maxPages) {
      ...
      // Issue 7 fix: derive hasPricing and hasDocs from the URL path
      let urlPath = ''
      ...
      try {
        ...
        try {
          const semanticsResult = await ctx.browser.evaluate(pageId, `(() => {
            ...
            // Issue 10 fix: collect up to 30 keys (aligned with storage node cap)
        ...
        // Issue 7 fix: pass URL-derived hasPricingUrl / hasDocsUrl instead of false, false
        const pageRole = inferPageRole(title, h1, hasPassword, hasPricingUrl, hasDocsUrl)

        // Issue 4 fix: store compact paths in apiCallsObserved (not full URLs)
        ...
        const { nodeId: mainNodeId } = await addNode(
          ...
          'page',
          {
            ...
            // Issue 4 fix: compact paths only, not full URLs
            ...
            if (Array.isArray(formsResult.value)) {
              ...
              for (let fi = 0; fi < forms.length; fi++) {
                ...
                if (form.action && (form.action.startsWith('http') || form.action.startsWith('/'))) {
                  // Issue 3 fix: use truncated label; full endpoint in meta
        ...
        // Issue 10 fix: cap raised to LOCAL_STORAGE_NODE_CAP (30) to match Phase 1 collection
        ...
        // Issue 3 fix: label is truncated; full URL lives in meta.endpoint
        for (const endpoint of apiCallsObserved.slice(0, 10)) {
          try {
            const { nodeId: acNodeId } = await addNode(
              apiCallLabel('GET', endpoint),
              'api_call',
              {
                parentPageId: mainNodeId,
                method: 'GET',
        ...
        if (depth < args.maxDepth) {
          ...
          for (const link of sameSiteLinks) {
            ...
            // Issue 6 fix: O(1) Set membership check instead of O(n) queue.includes()
            if (!bfsState.visited.has(link) && !bfsState.queued.has(link)) {
              bfsState.queued.add(link)
        ...
        // Issue 5 fix: per-page save is throttled (force=false); final save uses force=true
    ...
    // Issue 5 fix: force=true guarantees a final consistent JSON+MMD export
...
export const map_site_enqueue = defineTool({
  ...
  input: z.object({
    ...
    depth: z.coerce.number().int().min(0).optional()
      .describe('Depth to assign to this URL. Defaults to the current crawl maxDepth - 1 to prevent unbounded expansion.'),
  }),
  async handler(args, _ctx, response) {
    ...
    // Issue 11 fix: use provided depth, else assign maxDepth-1 (not 0)
    // to avoid triggering a full sub-crawl from a manually-added deep URL.
    const assignedDepth = args.depth ?? Math.max(0, bfsState.maxDepth - 1)
